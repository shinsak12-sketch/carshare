...
// 하는 게 목적이라 서로 역할이 다름.
export const PROCEDURE_PROMPT_VERSION_TAG = "p1.8";
...
특히 램프류(헤드램프·테일램프·방향지시등)는 렌즈 크랙·긁힘·파손이
사고 시 매우 흔한데도 곧잘 지나치는 부품이니 실제로 자세히 들여다보고
판단하십시오. 손상이 확인되면 damaged_parts에 추가하고, 정말 손상이
없다고 판단되면 damage_type을 "손상없음"으로 표시해 확인했다는 근거를
남기십시오. 검토도 안 해보고 형식적으로 "손상없음" 항목만 채워 넣는
것은 아무 의미가 없습니다 — 핵심은 실제로 자세히 보는 것이지, 항목을
만드는 것 자체가 아닙니다.
...
- 기준(중요): "좌"/"우"는 카메라가 놓인 위치나 사진 화면상 좌우가
  아니라, 항상 "차량 자체" 기준입니다 — 운전석 쪽이 좌측, 조수석
  쪽이 우측입니다(국내 자동차 좌측통행 기준, 운전석이 차량 왼쪽에
  있음). 예를 들어 차량 뒤쪽에서 정면을 바라보고 찍은 사진이면
  화면상 오른쪽에 보이는 손상이 실제로는 차량의 좌측(운전석 방향)
  입니다. 사진 속 다른 단서(운전석 유무, 사이드미러, 배기구 위치,
  주유구 등)로 이게 차량의 어느 쪽 면인지 먼저 판단한 뒤에 좌/우를
  결정하십시오.
