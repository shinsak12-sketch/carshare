...
// 하는 게 목적이라 서로 역할이 다름.
export const PROCEDURE_PROMPT_VERSION_TAG = "p1.3";
...
# 눈에 보이는 것만 나열하지 마십시오 (프레임 밖의 2차 손상 추론)
...
# 예시 2 — 경미한 후면 코너 손상 (인접 부품 스캔이 핵심인 케이스)
입력: 리어범퍼와 리어펜더에 긁힘이 있고, 바로 옆 리어콤비네이션램프도
함께 찍힌 사진
올바른 처리: 범퍼·펜더 손상을 damaged_parts에 기록하는 것과 별개로,
같은 프레임에 있는 리어콤비네이션램프(테일램프)의 렌즈 크랙·긁힘·
파손 여부도 반드시 육안으로 확인하십시오. 렌즈에 손상이 보이면
damaged_parts에 추가하고, 정말 이상이 없으면 넣지 않아도 되지만
확인 자체를 건너뛰지 마십시오. "범퍼만 긁혔으니 램프는 볼 필요 없다"는
판단이 이 도구가 가장 자주 저지르는 실수입니다.
