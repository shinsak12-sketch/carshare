...
// 청구 블록(C·D·F)은 쓰지 않음.
export const PROCEDURE_PROMPT_VERSION_TAG = "p3.0";
...
- 기준(중요): "좌/우"는 카메라나 화면 기준이 아니라 항상 차량 기준입니다
  (운전자가 앉아 앞을 볼 때의 좌우. 국내 차량은 운전석이 왼쪽). 사진마다
  먼저 촬영 방향을 정한 뒤 다음 규칙으로 환산하십시오.
...
# 수리 방법은 복원 가능 여부부터, 손상 강도에 비례해서
- 손상 부위마다 [판단 입장]의 생각 순서대로 정하십시오. 먼저 정비사 관점에서
  이 손상을 수리로 원래 형상·강성·단차까지 되돌릴 수 있는지 판단하고
  ([손상 판독과 수리·교환 판단]의 관찰 포인트), 되돌릴 수 없으면 교환입니다 —
  이때 reasoning에 왜 복원이 어려운지(가장자리 꺾임, 접합부 벌어짐, 파단 등)를
  적으십시오. 복원 가능하면 그때 수리 수준을 정합니다: 폴리싱(1유형) /
  보수도장(2유형) / PDR(도장 멀쩡한 완만한 함몰) / 판금+도장(3유형) /
  플라스틱 복원(범퍼 3유형).
...
1. damaged_parts: 사진에서 직접 확인되는 손상 부위와 경미손상 유형 판정, 그리고
   ...
   판단은 [손상 판독과 수리·교환 판단]의 판단 구간을 따르되 이 도구는
...
7. damaged_parts의 damage_type은 비워두지 말고 채우되, 복원 불가로 교환이 맞는
   ...
   [손상 판독과 수리·교환 판단]의 적용대상 부품에만 적용하고 그 외 부품(백패널·
   사이드멤버 등)에는 적용하지 마십시오. 판별이
