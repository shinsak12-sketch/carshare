...
// 하는 게 목적이라 서로 역할이 다름.
export const PROCEDURE_PROMPT_VERSION_TAG = "p1.5";
...
램프류(헤드램프·테일램프·방향지시등)는 특별 취급하십시오: 사고 시
렌즈 크랙·긁힘·파손이 매우 흔한데도 곧잘 지나치는 부품이라, 사진
프레임 안에 램프가 보이면 손상 여부와 무관하게 damaged_parts에
반드시 하나의 항목으로 포함시키십시오 — 손상이 있으면 해당 유형을,
없으면 damage_type을 "손상없음"으로 채우고 reasoning에 "렌즈 크랙·
파손 없음을 확인함"이라고 적으십시오. 프레임에 램프가 명확히 보이는데
damaged_parts에 그 램프 항목 자체가 없는 것은 틀린 출력입니다 — 검토를
생략했다는 뜻이기 때문입니다. (다른 부품들은 손상이 없으면 굳이
"손상없음" 항목을 만들지 않아도 되지만, 램프류만은 예외로 항상
명시하십시오.)
...
- 같은 프레임의 리어콤비네이션램프는 램프류이므로 damaged_parts에
  반드시 항목으로 포함시킵니다. 렌즈에 크랙·긁힘이 보이면 해당 유형을,
  안 보이면 damage_type을 "손상없음"으로 채우고 reasoning에 "렌즈 크랙·
  파손 없음을 확인함"이라고 적습니다 — 항목 자체를 빼놓지 않습니다.
