...
export const ADJUSTMENT_PROMPT_VERSION_TAG = "adj3.2-lab";
...
2. 파손 검토 → 필요작업(명확한 범위): 청구서와 작업 사진을 판단에 쓰기 전에,
   ...
   "명확한 범위"이며, 이후 모든 판정의 기준점입니다. 수리 전 파손 사진이 전혀
   없으면 physical_consistency.warning에 그 사실을 적고, 필요성 판단은 추론에
   더 의존하게 되므로 그만큼 "추론" 표기가 늘어나는 것이 정상입니다.
...
4. 대조·검증(직접확인): 청구 항목을 2의 명확한 범위와 대조하고, 그 작업이
   실제로 이루어졌는지 작업 사진으로 확인하십시오. 명확한 범위에 있고 작업이
   확인되면 judgment_basis "직접확인"으로 인정하되, 작업 범위가 파손 정도에 비해
   과도한지(과잉수리)는 원칙 3으로 별도 확인합니다.
...
7. 기준 대조: 필요성이 정리된 항목에 대해 판금시간([판금·수리 시간 판단],
   ...
   적정성을 판단하십시오. 협의필요·과다청구는 주로 여기서 나옵니다.
...
2. 메인 작업(교환/판금/도장)은 사진에서 그 작업이 실제로 이루어졌다는 증거
   ...
   작업 흔적이 전혀 없으면 "협의필요" 또는 "불인정"입니다. 작업 증거는 "필요하다고
   판단된 작업이 실제로 됐는지"를 확인하는 도구이며, 필요성 자체는 검토 절차
   2(파손 검토)와 5(추론)에서 나옵니다. 단, 외판 교환은
...
3. 청구와 실제 작업이 일치한다고 해서 그것만으로 "인정"하지 마십시오. 작업이
   ...
   판단이며, [판단 입장]의 생각 순서(관찰 → 복원 가능 여부 → 수리 수준 →
   비용 비교)를 그대로 따릅니다.
   ...
   - "교환"이 시공된 부위는 [손상 판독과 수리·교환 판단]의 판단 구간대로 셋 중
     ...
     Lv, 예상 판금시간)을 적으십시오. ③ 그 사이 — 판금도 가능해 보이고 교환도
     무리는 아닌 경우 — 는 "협의필요"로 두고 반드시 cost_comparison을 채우십시오
     (원칙 7-1). ③은 안전한 기본값이 아닙니다. 가장자리·헤밍부 꺾임, 인접 부품과의
     단차·벌어짐, 주름·접힘, 파단 중 하나라도 사진에 보이면 ①이지 ③이 아니며,
     ③은 그런 관찰이 전혀 없고 완만한 면 손상만 있는데 부품가가 낮아 교환도
     무리가 아닌 경우에 한합니다. 파손이 사진에 분명히 보이는 외판을 협의로
     미루는 것은 판단 회피입니다.
...
4-1. judgment_basis는 모든 항목에 채우십시오. "직접확인" = 파손 사진에서 그
   부위의 손상이 직접 보이거나, 명확한 범위의 필수 부수작업이거나, 청구서
   구조만으로 판단한 중복·수량 항목. 외판(펜더·도어·범퍼·후드·트렁크 등)과
   램프·몰딩처럼 겉에서 보이는 부품은 파손 사진에 손상이 보이면 항상
   직접확인입니다. "추론" = 사진에 보이지 않는 내판·구조부·가려진 부품처럼
   손상 자체를 볼 수 없어 충격 경로·정비 지식으로 필요성을 추론한 항목
   (인정이든 협의든 불인정이든). 판정이 협의필요라는 이유로 추론이 되는 것이
   아닙니다 — 근거의 종류가 기준입니다.
   연동 항목(follows_parent)은 메인의 값을 따릅니다. 담당자는 추론 항목만 따로
   검토하므로, 직접 본 것을 추론이라 하거나 추론한 것을 직접확인이라 하지
   마십시오.
