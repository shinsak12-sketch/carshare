...
export const PROMPT_VERSION_TAG = "v1.0";
...
1. [참고자료]에 없는 수치(표준작업시간, 부수작업 허용여부, 단가 등)는 추정하거나
   ...
   표시하십시오.
2. 사진만으로 손상 형태(소재 변형 vs 단순 도장손상)가 명확히 구분되지 않으면
   절대로 단정하지 말고 "협의대상"으로 분류한 뒤, 무엇을 추가로 확인해야
   하는지 구체적으로 적으십시오.
...
# 반드시 "협의대상"으로 분류해야 하는 경우 (하나라도 해당하면)
...
출력 예:
  ...
  reasoning: "패널이 벌어져 내측 브라켓이 노출되는 파단/이탈 형태로 확인되어
    경미손상 교환 인정 예외조건(파단·이중패널 이탈)에 해당함"
  verdict: "인정가능"

# 예시 2 — 애매한 케이스 (협의대상으로 처리)
입력: 펜더에 선형 스크래치가 있으나 함몰 여부가 불명확한 사진
출력 예:
  damage_type: "불명확(2유형 또는 3유형)"
  reasoning: "긁힘은 확인되나 소재 변형(함몰) 여부가 사진 각도상 판별 불가"
  verdict: "협의대상"
  required_action: "근접/측광 사진 추가 확보 필요"
    ...
    parts: {
      ...
      items: {
        ...
        properties: {
          ...
          damage_type: {
            type: "string",
            enum: ["1유형", "2유형", "3유형", "비대상(교환예외)", "불명확", "손상없음"],
          },
