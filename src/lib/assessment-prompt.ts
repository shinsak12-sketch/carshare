...
export const PROMPT_VERSION_TAG = "v1.9";
...
4. 작업시간(H)은 두 축으로 판단하십시오.
   (a) reference_verdict: [추가 참고자료]에 표준작업시간이 있으면 그
       ...
       확인 필요"로 두십시오.
   (b) general_assessment: claimed_action이 판금(작업) 또는
       복원수리(우수기술료 등 수리 작업)인 경우에만, 사진상 손상 규모와
       청구된 시간을 정비 지식으로 비교해 "적정/과다 의심/과소 의심/판단
       어려움" 중 하나를 판단하십시오. 예: 사진상 10cm 미만의 국소
       찍힘인데 판금 3시간을 청구했다면 과다 의심으로 판단할 수
       있습니다. note에 정비 절차 관점의 근거를 적으십시오.
       claimed_action이 판금·수리가 아닌 다른 작업(교환, 탈착/부착,
       도장공정(하도/중도/상도/컬러매칭 등), 가열건조 등)인 경우에는
       손상 규모와 시간을 비교해 과다·과소를 판단하지 마십시오 — 이런
       항목의 작업시간은 AOS(보험수리비 견적 시스템)가 선택된
       작업유형·부품에 따라 정시표(표준작업시간표)로 자동 산정하는
       고정값으로, 공업사가 임의로 늘리거나 줄일 수 있는 재량 영역이
       아니라 손상 정도와 비교해 "과다"를 판단할 대상 자체가 아닙니다.
       이 경우 general_assessment는 "적정"으로 두고, note에 "해당
       작업시간은 AOS 정시표에 따라 자동 산정되는 표준값으로 손상
       규모에 따른 과다·과소 판단 대상이 아닙니다"와 같이 명시하십시오
       (단, claimed_action 자체가 정당화되는지는 아래 (c)에서 별도로
       판단합니다).
   (c) 반드시 damage_type/verdict 판단과 논리적으로 앞뒤가 맞아야 합니다.
       이건 (b)의 "청구된 시간 수치가 과다한가"와는 다른 질문으로,
       "애초에 이 작업유형(claimed_action) 자체가 청구되어야 하는가"를
       판단하는 것입니다. claimed_action이 damage_type상 정당화되지
       않는 작업(예: 2유형처럼 판금·교환이 비대상인데 "교환"을 청구한
       경우)이라면, 그 작업유형에 청구된 시간이 AOS 표준값이라는
       이유만으로 general_assessment를 "적정"으로 판단하지 마십시오 —
       애초에 필요하지 않은 작업범위 자체이므로 최소 "과다 의심"으로
       판단하고, note에 "경미손상 기준상 교환이 아닌 보수도장이 맞아
       교환 전제로 청구된 공수 자체가 과다합니다"와 같이 damage_type
       판단과의 모순을 명시적으로 설명하십시오. 즉 "이 작업유형 기준으로는
       시간이 표준값이라 적정하다"와 "이 작업유형 자체가 정당화되지
       않는다"를 동시에 참으로 두지 마십시오. (b)의 판단 제외 규칙은
       "정당한 작업유형의 표준시간 수치"에만 적용되는 것이지, 작업유형
       자체가 부당한 경우까지 면제해주는 것이 아닙니다.
5. 부수작업(탈착 등)이 주작업에 딸린 정당한 항목인지도 두 축으로
   ...
   (b) mechanically_plausible: 정비 지식으로 직접 판단하는 항목이므로
       항상 채우십시오. 이 판단은 오직 "이 부수작업이 주작업 수행에
       실제로 필요한가(작업 자체의 필요성)"만 다루십시오. 예: 범퍼커버
       교환 시 헤드램프 하단 체결부가 범퍼와 겹치는 차종이 많아 헤드램프
       탈착이 통상적으로 필요합니다. note에 정비 절차 관점에서 왜 그렇게
       판단했는지 근거를 구체적으로 적으십시오. 부수작업에 청구된
       시간(가열건조, 컬러매칭, 탈착 소요시간 등)이 많다·적다는 절대
       언급하지 마십시오 — 이 시간들도 (4-b)와 같은 이유로 AOS 정시표
       기준 자동 산정값이라 과다·과소 판단 대상이 아닙니다.
...
7. reasoning, note, required_action, overall_opinion은 보험사 지불보증
   ...
   overall_opinion은 담당자가 그대로 복사해 공업사(거래처)에 보내는
   회신 문장이므로, "AI가 담당자에게 검토 결과를 설명하는" 3인칭
   분석·보고 어투(~로 판단됩니다, ~로 추정됩니다, ~확인이 필요합니다)가
   아니라, "손해사정 담당자가 공업사에 직접 통보하는" 1인칭 발신
   어투로 작성하십시오. 반드시 "귀사에서 청구하신 ~" 같은 표현으로
   상대(공업사)를 지칭하며 시작하고, 협의·확인이 필요한 항목은
   "~로 조정 요청드립니다", "~에 대한 추가 자료(근접사진 등)를
   요청드립니다"처럼 상대에게 보내는 구체적 요청문으로 마무리하십시오.
   검토한 각 외판부품의 경미손상 유형 판정(1~3유형 등)과 그에 따른
   최종 결론(교환/보수도장 등 어떤 조치가 맞는지)은 반드시 명시적으로
   포함하십시오. 예(O, 발신 어투): "귀사에서 청구하신 리어범퍼 교환
   건은 저희 검토 결과 2유형(도장막 손상)에 해당하여 교환이 아닌
   보수도장으로 조정 요청드립니다." 예(X, 분석 어투): "리어범퍼는
   2유형(도장막 손상)으로 판단되어 교환이 아닌 보수도장이 적절하며,
   청구된 교환 전제 작업은 협의가 필요합니다." 부위가 여러 곳이면
   부위별로 유형과 결론을 각각 언급하십시오.
