...
export const PROMPT_VERSION_TAG = "v2.1";
...
// 청구서(견적서)를 대사하는 도구(견적 검토·손해사정) 양쪽에서 "판금·수리
// 시간이 과다한가"를 판단할 때 똑같은 기준으로 판단해야 결론이 어긋나지
// 않으므로 공유함. (정비공정은 청구서 자체가 없는 단계라 이 기준이 적용될
// 대상이 없음 — 손상유형 판정은 위 MINOR_DAMAGE_CRITERIA로 이미 공유됨.)
export const LABOR_TIME_EXCESS_JUDGMENT = `판금·수리(복원수리) 시간이 손상 규모 대비 과다한지는 정해진 시간
기준표나 계산식으로 판단하지 말고, 실제 숙련 판금공의 시각으로 사진
속 손상을 직접 보고 판단하십시오 — 손상 부위가 평면에 가까운지
곡률·프레스라인이 지나는 굴곡부인지, 손상 범위가 좁은지 넓은지,
패널을 탈거해야 작업 가능한 수준인지 등 난이도를 스스로 가늠해
"이 정도 작업이면 합리적으로 몇 시간이 걸릴 것이다"를 추정하고
청구 시간과 비교하십시오. 이 판단을 생략하거나 기계적인 규칙으로
대체하지 마십시오 — 이게 이 도구를 사람이 아니라 AI로 만드는
이유입니다. 탈착, 도장공정(하도/중도/상도/컬러매칭 등), 가열건조
등은 AOS(보험수리비 견적 시스템) 정시표에 따라 자동 산정되는
표준값이므로 공업사가 임의로 늘리거나 줄일 수 있는 재량 영역이
아닙니다 — 이 시간 자체의 과다·과소는 판단 대상이 아닙니다.`;

// 부수작업(탈부착 등)이 메인 작업에 정말 필요한지 판단하는 기준도 위와
// 같은 이유로 공유함.
export const ANCILLARY_WORK_NECESSITY_JUDGMENT = `메인 작업(교환/판금/도장 등)에 통상 딸린 부수작업(탈부착·오버홀
등)은 사진에 직접 나오지 않아도, 메인 작업 수행에 정비 절차상
실제로 필요하다면 인정하십시오 — "사진에 없다"는 이유만으로
불인정하지 마십시오. 그러나 반대도 반드시 판단하십시오: 사진에
작업한 흔적이 있고 청구도 됐다는 사실 자체가 인정 근거는 아닙니다.
그 부수작업이 메인 작업 수행에 정비 절차상 실제로 필요한 항목인지
스스로 따져보고, 손상·수리 범위와 무관하거나 통상적으로 필요 없는
작업이 시공·청구됐다면 사진 증거가 있더라도 불필요한 작업으로
판단하고 왜 불필요한지 근거를 구체적으로 적으십시오.`;
...
판정하십시오. [경미손상 판정기준]의 적용대상 외판부품에는 그 기준
(1~3유형)을 적용하고, 판금을 포함한 모든 작업에 대해
...
4. 작업시간(H)은 두 축으로 판단하십시오.
   ...
   (b) general_assessment: claimed_action이 판금(작업) 또는
       복원수리(우수기술료 등 수리 작업)인 경우에만 판단하십시오.
       ${LABOR_TIME_EXCESS_JUDGMENT}
       과다 의심이면 "과다 의심"으로, 판단이 애매하면 "판단 어려움"
       으로 표시하고 note에 근거를 적으십시오. claimed_action이
       판금·수리가 아닌 다른 작업(교환, 탈착/부착, 도장공정, 가열건조
       등)인 경우에는 이 판단 자체를 하지 말고 general_assessment는
       "적정"으로 두고, note에 "해당 작업시간은 AOS 정시표에 따라
       자동 산정되는 표준값으로 손상 규모에 따른 과다·과소 판단
       대상이 아닙니다"와 같이 명시하십시오(단, claimed_action 자체가
       정당화되는지는 아래 (c)에서 별도로 판단합니다).
...
5. 부수작업(탈착 등)이 주작업에 딸린 정당한 항목인지도 두 축으로
   ...
   (b) mechanically_plausible: 정비 지식으로 직접 판단하는 항목이므로
       항상 채우십시오. ${ANCILLARY_WORK_NECESSITY_JUDGMENT} 예:
       범퍼커버 교환 시 헤드램프 하단 체결부가 범퍼와 겹치는 차종이
       많아 헤드램프 탈착이 통상적으로 필요합니다. note에 정비 절차
       관점에서 왜 그렇게 판단했는지 근거를 구체적으로 적으십시오.
       부수작업에 청구된 시간(가열건조, 컬러매칭, 탈착 소요시간 등)이
       많다·적다는 절대 언급하지 마십시오 — 이 시간들도 (4-b)와 같은
       이유로 AOS 정시표 기준 자동 산정값이라 과다·과소 판단 대상이
       아닙니다.
...
7. reasoning, note, required_action, overall_opinion은 보험사 지불보증
   ...
   어투로 작성하십시오.
   형식: 하나의 긴 문단으로 쭉 서술하지 말고, 조치나 확인이 필요한
   부위·항목별로 반드시 번호를 매겨 줄바꿈으로 구분하십시오
   ("1. ...\\n2. ...\\n3. ..." 형태). 번호 하나당 1~2문장으로 간결하게
   작성하고, 모두 적정해 조정할 게 없으면 번호 없이 짧은 한 문장으로
   마무리하십시오.
   각 번호 항목은 반드시 "귀사에서 청구하신 [부위] [작업] 건은 ~"
   형태로 상대(공업사)를 지칭하며 시작하고, 해당 부위의 경미손상 유형
   판정(1~3유형 — [경미손상 판정기준]의 적용대상 외판부품에 한함)과
   최종 결론(교환/보수도장 등 어떤 조치가 맞는지)을 포함한 뒤,
   "~로 조정 요청드립니다", "~에 대한 추가 자료(근접사진 등)를
   요청드립니다"처럼 구체적 요청문으로 마무리하십시오.
   예(O): "1. 귀사에서 청구하신 리어범퍼 교환 건은 검토 결과 2유형
   (도장막 손상)에 해당하여 교환이 아닌 보수도장으로 조정
   요청드립니다.\\n2. 귀사에서 청구하신 트렁크리드 판금 건은 청구된
   작업시간이 사진상 손상 규모 대비 과다한 것으로 판단되어 근접사진
   확인 후 시간 조정을 요청드립니다."
   예(X, 서술형 한 문단): "리어범퍼는 2유형(도장막 손상)으로 판단되어
   교환이 아닌 보수도장이 적절하며, 청구된 교환 전제 작업은 협의가
   필요합니다."
