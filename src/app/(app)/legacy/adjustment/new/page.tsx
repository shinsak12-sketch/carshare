...
export default function NewAdjustmentPage() {
  ...
  function storeResult(caseId: string, r: AdjustmentResult) {
    setCases((prev) => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = await postToolForm<AdjustmentResult>(
        "/api/adjustment",
        formData,
        "청구 견적서(PDF)",
      );
      const r = await runAiJob<AdjustmentResult>(
        data,
        imageUrls,
        setLoadingStep,
  ...
  async function handleImportJson(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const r = (parsed?.result ?? parsed) as AdjustmentResult;
      if (!r || !Array.isArray(r.items) || !r.physical_consistency) {
        throw new Error("손해사정 결과 JSON 형식이 아닙니다.");
      }
        ...
        <div className="mb-3 flex shrink-0 flex-wrap items-end justify-between gap-x-4 gap-y-2">
          <div className="flex min-w-0 items-end gap-4">
            <h1 className="shrink-0 text-2xl font-bold text-slate-900 lg:text-3xl">
              AI 손해사정
              <span className="ml-2 rounded-full bg-slate-200 px-2 py-0.5 align-middle text-xs font-bold text-slate-600">
                구버전
              </span>
            </h1>
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            {loading && (
              <div className="flex min-w-0 max-w-full items-center gap-2 rounded-full border border-purple-200 bg-purple-50 px-4 py-2 text-xs font-bold text-purple-700 sm:text-sm">
