...
} from "@/lib/adjustment-lab-prompt";
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[/api/adjustment-lab] failed:", err);
    const message =
...
async function handleAdjustment(req: NextRequest) {
  ...
  const runInput = {
    ...
    promptVersion:
      taggedPromptVersion(ADJUSTMENT_PROMPT_VERSION_TAG, aiModel.detail) +
  ...
  after(async () => {
    await sweepStaleBlobs("/api/adjustment-lab");
  });
